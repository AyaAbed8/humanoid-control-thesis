function results = calibrate_sagittal_push_response( ...
    forceMagnitude_N, stopTime_s, outputRoot, pulseStart_s, pulseDuration_s)
%CALIBRATE_SAGITTAL_PUSH_RESPONSE Compare A/D under sagittal pushes.
%   RESULTS = CALIBRATE_SAGITTAL_PUSH_RESPONSE() runs six cases:
%     1: A, zero force       4: D, zero force
%     2: A, +y force        5: D, +y force
%     3: A, -y force        6: D, -y force
%
...
arguments
    forceMagnitude_N (1,1) double {mustBeFinite, mustBeNonnegative} = 50
...
assert(pulseStart_s + pulseDuration_s < stopTime_s, ...
    "SagittalCalibration:PulseOutsideRun", ...
...
assert(isfile(modelFile), "SagittalCalibration:ModelMissing", ...
...
caseTable = table( ...
    (1:6).', ["A"; "A"; "A"; "D"; "D"; "D"], ...
    ["nominal"; "positive_y"; "negative_y"; ...
     "nominal"; "positive_y"; "negative_y"], ...
    [0; forceMagnitude_N; -forceMagnitude_N; ...
     0; forceMagnitude_N; -forceMagnitude_N], ...
    'VariableNames', ...
    {'CaseNumber', 'Controller', 'Condition', 'ForceY_N'});
...
for caseIndex = 1:height(caseTable)
    ...
    forceVector = [0; definition.ForceY_N; 0];
    fprintf("[%d/6] Controller %s, %s, Fy = %+.6g N\n", ...
        caseIndex, definition.Controller, ...
        definition.Condition, definition.ForceY_N);
    ...
    assert(any(strcmp(output.who, "logsout")), ...
        "SagittalCalibration:MissingLogsout", ...
        "Case %d returned no logsout dataset.", caseIndex);
    assertRequiredLogs(output.logsout, requiredLogs, caseIndex);

    nominalReference = [];
    if definition.ForceY_N ~= 0
        if definition.Controller == "A"
            nominalReference = trajectories{1};
        else
            nominalReference = trajectories{4};
        end
        assert(~isempty(nominalReference), ...
            "SagittalCalibration:NominalReferenceMissing", ...
            "The matched zero-force trajectory must run before disturbances.");
    end
    [metricRows{caseIndex}, trajectories{caseIndex}] = ...
        processCase(output.logsout, definition, ...
        stopTime_s, pulseStart_s, pulseDuration_s, nominalReference);
end
...
assert(getSimulinkBlockHandle(forceBlock) > 0, ...
    "SagittalCalibration:ForceInterfaceMissing", ...
...
assert(strcmp(get_param(forceBlock, "EnableForce"), "on") && ...
    ...
    "SagittalCalibration:ForceInterfaceInvalid", ...
...
assert(isscalar(contactLines), ...
    "SagittalCalibration:ContactSignalNotUnique", ...
...
% qx is the sagittal torso-tilt coordinate for a y-direction push.
torsoY = q(:, 2);
torsoSagittalTilt = q(:, 4);
legAngles = q(:, 11:22);
torsoYDeviation = torsoY - torsoY(1);
torsoSagittalTiltDeviation = torsoSagittalTilt - torsoSagittalTilt(1);
legDeviation = legAngles - legAngles(1, :);

% For a disturbed run, remove the matched controller-specific zero-force
% trajectory at the same time. Sagittal standing has a non-negligible
% nominal transient, so initial-pose subtraction alone would attribute
% ordinary nominal motion to the force pulse.
if ~isempty(nominalReference)
    torsoYDeviation = torsoYDeviation - interpolateMatrix( ...
        nominalReference.qTime, nominalReference.torsoYDeviation, qTime);
    torsoSagittalTiltDeviation = torsoSagittalTiltDeviation - ...
        interpolateMatrix(nominalReference.qTime, ...
        nominalReference.torsoSagittalTiltDeviation, qTime);
    legDeviation = legDeviation - interpolateMatrix( ...
        nominalReference.qTime, nominalReference.legDeviation, qTime);
end
torsoYVelocity = numericalDerivative(qTime, torsoY);
if ~isempty(nominalReference)
    torsoYVelocity = numericalDerivative(qTime, torsoYDeviation);
end
...
earlyYRms = timeRms(qTime(earlyMask), ...
    torsoYDeviation(earlyMask));
lateYRms = timeRms(qTime(lateMask), ...
    torsoYDeviation(lateMask));
earlyTiltRms = timeRms(qTime(earlyMask), ...
    torsoSagittalTiltDeviation(earlyMask));
lateTiltRms = timeRms(qTime(lateMask), ...
    torsoSagittalTiltDeviation(lateMask));
yRatio = safeRatio(lateYRms, earlyYRms);
...
pattern = exploratoryPattern( ...
    definition.ForceY_N, yRatio, tiltRatio, ...
    torsoYDeviation(end), max(abs(torsoYDeviation)));

row = [definition, table( ...
    finalTime, reachedStopTime, ...
    rad2deg(max(abs(torsoSagittalTiltDeviation))), ...
    max(abs(torsoYDeviation)), torsoYDeviation(end), ...
    torsoYVelocity(end), ...
    rmsAll(rad2deg(legDeviation)), ...
    max(abs(rad2deg(legDeviation)), [], "all"), ...
    earlyYRms, lateYRms, yRatio, ...
    ...
    'PeakTorsoSagittalTiltDeviation_deg', ...
    'PeakTorsoYDeviation_m', 'FinalTorsoYDeviation_m', ...
    'FinalTorsoYVelocity_m_s', ...
    'PostureRMSError_deg', 'MaxJointDeviation_deg', ...
    'EarlyTorsoYRMS_m', 'LateTorsoYRMS_m', ...
    'TorsoYLateEarlyRatio', ...
    'EarlyTorsoSagittalTiltRMS_deg', 'LateTorsoSagittalTiltRMS_deg', ...
    'TorsoSagittalTiltLateEarlyRatio', ...
...
trajectory = struct( ...
    ...
    "qTime", qTime, ...
    "q", q, ...
    "torsoYDeviation", torsoYDeviation, ...
    "torsoSagittalTiltDeviation", torsoSagittalTiltDeviation, ...
    "torsoYVelocity", torsoYVelocity, ...
...
function pattern = exploratoryPattern( ...
    forceY, yRatio, tiltRatio, finalY, peakY)
if forceY == 0
    pattern = "nominal reference";
    return
end
growthRatio = max(yRatio, tiltRatio);
if growthRatio > 1.25 && abs(finalY) > 0.5 * peakY
...
assert(isempty(missing), "SagittalCalibration:MissingLogs", ...
...
assert(~isempty(time) && ~isempty(values), ...
    "SagittalCalibration:EmptySignal", ...
...
assert(size(values, 1) == numel(time), ...
    "SagittalCalibration:SignalShape", ...
...
error("SagittalCalibration:UnsupportedLogType", ...
...
assert(size(values, 2) == width, ...
    "SagittalCalibration:UnexpectedWidth", ...
...
figureHandle = figure( ...
    "Name", "Sagittal push calibration", ...
...
for index = 1:numel(trajectories)
    ...
    nexttile(1);
    plot(trajectory.qTime, trajectory.torsoYDeviation, ...
    ...
    plot(trajectory.qTime, ...
        rad2deg(trajectory.torsoSagittalTiltDeviation), ...
        "LineWidth", 1.2, "Color", colors(index, :), ...
        "DisplayName", label);
    hold on

    nexttile(3);
    plot(trajectory.qTime, trajectory.torsoYVelocity, ...
        "LineWidth", 1.2, "Color", colors(index, :), ...
        "DisplayName", label);
    hold on
    ...
    nexttile(6);
    plot(trajectory.forceTime, trajectory.force(:, 2), ...
        "LineWidth", 1.2, "Color", colors(index, :), ...
        "DisplayName", label);
    hold on
end
...
titles = [ ...
    "Sagittal torso displacement"
    "Torso sagittal-tilt deviation"
    "Sagittal torso velocity"
    "Lower of the two foot loads"
    "Balance-feedback torque norm"
    "Commanded world-y force"];
...
title(layout, "Frozen-controller sagittal push calibration");
exportgraphics(figureHandle, ...
    fullfile(outputDirectory, "sagittal_push_calibration.png"), ...
    "Resolution", 200);
savefig(figureHandle, ...
    fullfile(outputDirectory, "sagittal_push_calibration.fig"));
close(figureHandle);
end
